...
-   FR-032: Edytor piosenek obsługuje składnię do oznaczania powtórzeń wielowierszowych w formacie `xN(L)`, gdzie `N` to liczba powtórzeń, a `L` to liczba wierszy objętych powtórzeniem. Składnia ta jest konwertowana do formatu ChordPro `{block_start: xN}` i `{block_end}`.
-   FR-033: W widoku "Biesiada" oraz w widoku publicznym, bloki powtórzeń (zarówno jedno-, jak i wielowierszowe) są wizualnie oznaczane za pomocą pionowej linii z prawej strony, obejmującej wszystkie powtarzane wiersze, oraz wskaźnika `× N` przy ostatnim wierszu bloku.
